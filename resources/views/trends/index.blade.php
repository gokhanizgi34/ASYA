<x-layouts.app title="Trend Motoru">
    <section class="space-y-7">
        ...
        <nav class="grid grid-cols-2 gap-2 rounded-xl border border-white/10 bg-slate-950/60 p-1" aria-label="Trend kaynağı">
            <a href="{{ route('trends.index', ['provider' => 'google']) }}" class="px-4 py-3 text-center font-bold {{ $provider === 'google' ? 'rounded-lg bg-fuchsia-300 text-slate-950' : 'text-slate-300 hover:text-white' }}">Google Trends <span class="ml-2 text-sm opacity-70">{{ $providerCounts['google'] }}</span></a>
            <a href="{{ route('trends.index', ['provider' => 'x']) }}" class="px-4 py-3 text-center font-bold {{ $provider === 'x' ? 'rounded-lg bg-cyan-300 text-slate-950' : 'text-slate-300 hover:text-white' }}">X Gündemi <span class="ml-2 text-sm opacity-70">{{ $providerCounts['x'] }}</span></a>
        </nav>
        @if($provider === 'google')
