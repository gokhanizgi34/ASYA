...
<p class="break-all rounded-xl bg-slate-900/70 p-3"><span class="block text-xs text-slate-500">Search Console mülkü</span>{{ $integration->username }}</p>
<p class="break-all rounded-xl bg-slate-900/70 p-3"><span class="block text-xs text-slate-500">Haber site haritası</span>{{ $integration->model }}</p>
</div>
@else
...
@if($integration->last_error)<p class="mt-3 rounded-xl border border-rose-400/20 bg-rose-400/5 p-3 text-sm text-rose-300">{{ $integration->last_error }}</p>@endif
