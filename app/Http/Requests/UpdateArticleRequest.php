...
class UpdateArticleRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'slug' => ['required', 'string', 'max:255', Rule::unique('articles', 'slug')->where(fn ($query) => $query->where('agency_id', $this->input('agency_id')))->ignore($article)],
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            ...
            if ($role === UserRole::Editor && ! in_array($status, [ArticleStatus::Draft, ArticleStatus::PendingApproval], true)) {
                $validator->errors()->add('status', 'Editör yalnızca taslak veya onay bekleyen içerik kaydedebilir.');
            }
    ...
    protected function prepareForValidation(): void
    {
        ...
        $agencyId = $this->user()?->isSystemAdministrator()
            ...
            : ($article instanceof Article ? $article->agency_id : $this->user()?->agency_id);
